...
`realloc` の罠（L29）は特に悪質です。`*buf = realloc(*buf, *cap)` と書いた瞬間、`realloc` が失敗した場合に元のポインタが上書きされて失われます。`free` もできず、NULL デリファレンスも起きる、という二重の事故になります。自分が最初に見たのは組み込みのプロジェクトで、「なぜメモリが増えるのにクラッシュするのか」を追いかけました。

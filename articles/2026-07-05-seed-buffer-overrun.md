...
先週入った案件で、`sizeof(p)` を「バッファ全体のサイズ」だと思い込んで `memset(p, 0, sizeof(p))` と書かれたコードに遭遇しました。ポインタ引数の sizeof は 8（64bit環境）を返すだけで、確保した領域のクリアにはなっていません。本人に指摘したら「え、sizeof って便利なマクロじゃないんですか」と真顔で返ってきて、説明に時間を使いました。今回はこの手のバグを含む4関数のサンプルコードを creview に食わせて、検出結果を実際に見ていきます。

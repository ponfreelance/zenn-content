...
C言語23年、自分はフリーランスで組み込みとサーバーサイドC両方のコードレビューに入ることが多いです。若手の PR でよく見る「やらかし」の筆頭が `strcpy` と `sprintf` と `sizeof(ポインタ)` の3点セットで、これらは毎週のように別の PR で再登場します。先週指摘したはずの `strcpy` が、週をまたいで別の関数で出てくる。自分は1つの PR で同じ指摘を繰り返し書いたことがあります。人間のレビューで全部拾い続けるのは現実的ではないと判断して、自社プロダクトの `creview` に押し付けることにしました。
